<p align="center">
  <img src="logo.png" alt="VulnHunter" width="280"/>
</p>
...
<p align="center">
  <a href="https://github.com/shuvonsec/Nemo-bugbounty/blob/main/LICENSE"><img src="https://img.shields.io/badge/License-MIT-yellow.svg?style=flat-square" alt="MIT License"></a>
  ...
  <a href="https://star-history.com/#shuvonsec/Nemo-bugbounty"><img src="https://img.shields.io/github/stars/shuvonsec/Nemo-bugbounty?style=flat-square&color=yellow" alt="GitHub Stars"></a>
</p>

<p align="center">
  <a href="https://star-history.com/#shuvonsec/Nemo-bugbounty&Date">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=shuvonsec/Nemo-bugbounty&type=Date&theme=dark" />
      <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=shuvonsec/Nemo-bugbounty&type=Date" />
      <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=shuvonsec/Nemo-bugbounty&type=Date" />
    </picture>
  </a>
</p>
...
Works as a [Claude Code](https://claude.ai/claude-code) plugin **or** as a fully standalone CLI (`vulnhunter`) powered by free AI providers.
...
Install once, use the `vulnhunter` command from any terminal on your machine:

```bash
git clone https://github.com/shuvonsec/Nemo-bugbounty.git
cd Nemo-bugbounty
./install.sh --agent standalone
```

```
vulnhunter help          # show every command
vulnhunter setup         # choose your AI provider (Ollama is free + offline)
vulnhunter recon target.com   # map the attack surface
vulnhunter hunt target.com    # hunt for vulnerabilities
vulnhunter validate "finding" # 7-Question Gate on your finding
vulnhunter report        # write a submission-ready report
vulnhunter chat          # interactive AI hunting shell
vulnhunter providers     # list all available AI providers
vulnhunter status        # check which provider is active
vulnhunter h target.com  # short alias for hunt
vulnhunter r target.com  # short alias for recon
vulnhunter v "finding"   # short alias for validate
```
...
VulnHunter auto-detects providers in this order: **Ollama → Groq → DeepSeek → Claude → OpenAI**

Switch providers anytime: `vulnhunter setup`
...
# 2. Install VulnHunter
git clone https://github.com/shuvonsec/Nemo-bugbounty.git
cd Nemo-bugbounty
./install.sh --agent standalone   # creates system-wide 'vulnhunter' command

# 3. Hunt
vulnhunter setup         # choose Ollama
vulnhunter recon target.com
```
...
./install.sh --agent standalone
vulnhunter setup         # choose Groq
vulnhunter hunt target.com
```
...
cd Nemo-bugbounty
./install.sh --agent standalone   # creates system-wide 'vulnhunter' command
vulnhunter setup         # pick a free AI provider
vulnhunter recon target.com
vulnhunter hunt target.com
vulnhunter validate "my finding"
vulnhunter report
```
...
```text
Install the Nemo-bugbounty toolkit from https://github.com/shuvonsec/Nemo-bugbounty
...
│
├── engine.py                   # Standalone CLI — 'vulnhunter' command, no subscription needed
...
**Standalone `vulnhunter` command** (no subscription, works without Claude Code):

```bash
./install.sh --agent standalone
vulnhunter setup   # choose Ollama (free) · Groq (free tier) · DeepSeek (cheap) · Claude · OpenAI
```
...
<a href="https://www.star-history.com/?repos=shuvonsec%2FNemo-bugbounty&type=date&legend=top-left">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=shuvonsec/Nemo-bugbounty&type=date&theme=dark&legend=top-left" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=shuvonsec/Nemo-bugbounty&type=date&legend=top-left" />
    <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=shuvonsec/Nemo-bugbounty&type=date&legend=top-left" />
  </picture>
</a>
